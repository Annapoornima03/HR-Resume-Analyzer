...
class LLMResumeAnalyzer:
    ...
    # Poppler path: use bundled binaries on Windows, system poppler on Linux (Streamlit Cloud)
    _local_poppler = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "poppler", "Library", "bin"
    )
    POPPLER_PATH = _local_poppler if os.path.isdir(_local_poppler) else None
    ...
    def extract_text_from_pdf(self, uploaded_file):
        ...
        # Trigger OCR whenever extracted text looks insufficient
        if len(text.strip()) < 200:
            if ocr_reader:
                ocr_result = self._force_ocr(bytes_data)
                if len(ocr_result.strip()) > len(text.strip()):
                    text = ocr_result
            else:
                logging.warning(
                    "OCR reader unavailable; text may be incomplete. "
                    "Install EasyOCR for better scanned-PDF support."
                )

        return text.strip()

    def _force_ocr(self, bytes_data: bytes) -> str:
        """Run OCR on every page of a PDF regardless of existing text length."""
        try:
            poppler_path = self.POPPLER_PATH if os.path.isdir(self.POPPLER_PATH) else None
            images = pdf2image.convert_from_bytes(bytes_data, poppler_path=poppler_path)
            ocr_text: List[str] = []
            for img in images:
            ...
            logging.warning(f"Force OCR failed: {e}")
            return ""
                ...
                if not jd_text:
                    ...
                    for idx, file in enumerate(uploaded_files):
                        ...
                        try:
                            ...
                            # If still not enough text after OCR, try one explicit OCR pass
                            if not text or len(text.strip()) < 100:
                                if ocr_reader:
                                    bytes_data = file.getvalue()
                                    ocr_result = st.session_state.analyzer._force_ocr(bytes_data)
                                    if len(ocr_result.strip()) > len(text.strip() if text else ""):
                                        text = ocr_result

                            # Still low quality — use best-effort instead of skipping
                            if not text or len(text.strip()) < 50:
                                st.warning(
                                    f"⚠️ Very little text extracted from **{file.name}** "
                                    f"(possibly a scanned/image PDF). "
                                    f"Proceeding with best-effort analysis."
                                )
                                text = f"Resume filename: {file.name}"
                                low_confidence = True
                            elif len(text.strip()) < 200:
                                st.info(
                                    f"ℹ️ Limited text extracted from **{file.name}** — "
                                    f"analysis may be less accurate."
                                )
                                low_confidence = True
                            ...
                            elif final_score >= st.session_state.config["SCORE_THRESHOLD"]:
